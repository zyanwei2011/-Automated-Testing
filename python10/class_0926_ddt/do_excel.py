...
class DoExcel:
    def do_excel(self,button,case_id_list):
        ...
        for i in range(2,8):#i=2
            ...
            test_data.append(sub_data)
            #到此为止 所有的测试数据 已经处理完毕 都存在test_data

        if button=='on':#默认执行所有的用例
            final_data=test_data#最终要执行的用例数据放到final_data变量里面
        else:#执行指定列表case_id_list里面的数据
            final_data=[]#用例存储最终执行的用例
            for item in test_data:#列表的遍历
...
if __name__ == '__main__':
   test_data=DoExcel().do_excel("on",[1,3])
